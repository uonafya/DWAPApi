...
def map_data(request):
    ...
    try:
        for j in moh_csv_dict:
            if len(indicators.objects.filter(Q(MOH_Indicator_ID__icontains=str(j['MOH_Indicator_ID'])))) > 0:
                mohdata = []
            else:
    ...
    try:
        for i, d in enumerate(datimydict):
            for j, m in enumerate(mohdict):
                if 'Completed IPT_12months' in m['MOH_Indicator_Name']:
                    if 'TB_PREV' in d['DATIM_Indicator_Category'] and ('<' in d['DATIM_Disag_Name'] or '+' in d['DATIM_Disag_Name']) and ('Female' in d['DATIM_Disag_Name'] or 'Male' in d['DATIM_Disag_Name'] or 'Unknown Sex' in d['DATIM_Disag_Name']) and ('Newly Enrolled' in d['DATIM_Disag_Name'] or 'Previously Enrolled' in d['DATIM_Disag_Name']):
        ...
        for i, j in enumerate(final_dict):
            ...
            print(final_data)
        temp_df.to_csv(os.path.join(os.path.join(ABSOLUTE_PATH(), 'media\\mapped'),
